...
wspr_msg_start(wspr_msg_t id)
{
  ...
  sdPut(SD_WIFI, WSPR_SYNC1_CHAR);
  sdPut(SD_WIFI, WSPR_SYNC2_CHAR);
  sdPut(SD_WIFI, id);
...
wspr_msg_end()
{
  ...
  sdPut(SD_WIFI, data_len);
  sdPut(SD_WIFI, data_len >> 8);
  if (data_len > 0)
    sdWrite(SD_WIFI, msg_data->buf, data_len);
...
recv_wspr_pkt(void* arg, wspr_msg_t id, uint8_t* data, uint16_t data_len)
{
  ...
  if (handler)
    ...
    chprintf(SD_STDIO, "no handler for pkt id %d\r\n", id);
}
